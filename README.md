...
`index.html`をブラウザで開くと、同じHTMLリスト変換をブラウザ上で実行できます。Pythonや`pybtex`のインストールは不要です。

1. `index.html`を開く。
2. BibTeXフォルダ、複数の`.bib`ファイル、または貼り付けテキストを指定する。
3. 「変換」を押す。
4. 生成されたHTMLをコピーするか、`output.html`として保存する。

ブラウザ版もPython版と同様に、ファイル名を逆順に処理し、`<ul>`と`<li>`で構成されたHTMLを出力します。
